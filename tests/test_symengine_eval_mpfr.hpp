...
TEST(symengineEvalMPFR, eval_mpfr)
{
	mpfr_t a;
	mpfr_init2(a, 53);
	...
	EXPECT_TRUE(mpfr_cmp_si(a, 0) == 1);

	// Check that value of `r` (`a`) starts with 0.000000000149734291
	EXPECT_TRUE(mpfr_cmp_d(a, 0.000000000149734291) == 1);
	EXPECT_TRUE(mpfr_cmp_d(a, 0.000000000149734292) == -1);
