...
TEST(symengineEvalDouble, eval_double)
{
	...
	for (unsigned i = 0; i < vec.size(); i++) {
		...
		EXPECT_TRUE(::fabs(val - vec[i].second) < 1e-12);
	}
	...
	EXPECT_THROW(eval_double(*symbol("x")), std::runtime_error);
	EXPECT_THROW(eval_double_single_dispatch(*symbol("x")), std::runtime_error);

	// TODO: this is not implemented yet, so we check that it raises an
	// exception for now
	EXPECT_THROW(eval_double(*levi_civita({r1})), std::runtime_error);
	EXPECT_THROW(eval_double_single_dispatch(*levi_civita({r1})), std::runtime_error);

	EXPECT_THROW(eval_double(*zeta(r1, r2)), std::runtime_error);
	EXPECT_THROW(eval_double_single_dispatch(*zeta(r1, r2)), std::runtime_error);
	// ... we don't test the rest of functions that are not implemented.
}

TEST(symengineEvalDouble, eval_complex_double)
{
	RCP<const Basic> r1, r2, r3, r4;
	...
	for (unsigned i = 0; i < vec.size(); i++) {
		...
		//std::cout.precision(12);
		//std::cout << vec[i].first->__str__() << " ~ " << val << std::endl;
		EXPECT_TRUE(std::abs(val.imag() - vec[i].second.imag()) < 1e-12);
		EXPECT_TRUE(std::abs(val.real() - vec[i].second.real()) < 1e-12);
	}
}
