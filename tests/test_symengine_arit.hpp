...
TEST(symengineArit, Add)
{
	...
	EXPECT_TRUE(neq(*r1, *r3));

	r3 = mul(i2, y);
	EXPECT_TRUE(neq(*r1, *r3));
	EXPECT_TRUE(neq(*r2, *r3));
	...
	r3 = mul(mul(I, i3), x);
	r2 = add(r1, r2);
	EXPECT_TRUE(eq(*r3, *r2));

	r1 = real_double(0.1);
	r2 = Rational::from_mpq(mpq_class(1, 2));
	r3 = add(add(add(r1, r2), integer(1)), real_double(0.2));
	EXPECT_TRUE(is_a<RealDouble>(*r3));
	EXPECT_TRUE(std::abs(rcp_static_cast<const RealDouble>(r3)->i - 1.8) < 1e-12);
	...
	EXPECT_TRUE(is_a<ComplexDouble>(*r3));
	EXPECT_TRUE(std::abs(rcp_static_cast<const ComplexDouble>(r3)->i.real() - 2.0) < 1e-12);
	EXPECT_TRUE(std::abs(rcp_static_cast<const ComplexDouble>(r3)->i.imag() - 1.6) < 1e-12);
}

TEST(symengineArit, Mul)
{
	RCP<const Basic> x = symbol("x");
	RCP<const Basic> y = symbol("y");
	RCP<const Basic> z = symbol("z");
	...
	EXPECT_TRUE(is_a<RealDouble>(*r2));
	EXPECT_TRUE(std::abs(rcp_static_cast<const RealDouble>(r2)->i - 0.03) < 1e-12);

	r1 = complex_double(std::complex<double>(0.1, 0.2));
	r2 = Complex::from_two_nums(*Rational::from_mpq(mpq_class(1, 2)), *Rational::from_mpq(mpq_class(7, 5)));
	r2 = mul(mul(mul(r1, r2), integer(5)), real_double(0.7));
	EXPECT_TRUE(is_a<ComplexDouble>(*r2));
	EXPECT_TRUE(std::abs(rcp_static_cast<const ComplexDouble>(r2)->i.real() + 0.805) < 1e-12);
	EXPECT_TRUE(std::abs(rcp_static_cast<const ComplexDouble>(r2)->i.imag() - 0.84) < 1e-12);
...
TEST(symengineArit, Sub)
{
	...
	EXPECT_TRUE(is_a<RealDouble>(*r2));
	EXPECT_TRUE(std::abs(rcp_static_cast<const RealDouble>(r2)->i + 3.6) < 1e-12);

	r1 = real_double(0.1);
	r2 = Complex::from_two_nums(*Rational::from_mpq(mpq_class(1, 2)), *Rational::from_mpq(mpq_class(7, 5)));
	r2 = sub(sub(sub(r1, r2), integer(1)), real_double(0.4));
	EXPECT_TRUE(is_a<ComplexDouble>(*r2));
	EXPECT_TRUE(std::abs(rcp_static_cast<const ComplexDouble>(r2)->i.real() + 1.8) < 1e-12);
	EXPECT_TRUE(std::abs(rcp_static_cast<const ComplexDouble>(r2)->i.imag() + 1.4) < 1e-12);
}

TEST(symengineArit, Div)
{
	RCP<const Basic> x = symbol("x");
	RCP<const Basic> y = symbol("y");
	RCP<const Basic> z = symbol("z");
	...
	EXPECT_TRUE(integer(2)->is_positive());
	EXPECT_TRUE(integer(0)->is_zero());
	EXPECT_TRUE(integer(1)->is_one());
	EXPECT_TRUE(!(integer(-1)->is_positive()));
	EXPECT_TRUE(integer(-1)->is_negative());
	...
	EXPECT_TRUE(is_a<RealDouble>(*r2));
	EXPECT_TRUE(std::abs(rcp_static_cast<const RealDouble>(r2)->i - 0.333333333333) < 1e-12);

	r1 = real_double(0.1);
	r2 = Complex::from_two_nums(*Rational::from_mpq(mpq_class(1, 2)), *Rational::from_mpq(mpq_class(7, 5)));
	r2 = div(div(div(r1, r2), integer(2)), real_double(0.4));
	EXPECT_TRUE(is_a<ComplexDouble>(*r2));
	EXPECT_TRUE(std::abs(rcp_static_cast<const ComplexDouble>(r2)->i.real() - 0.0282805429864253) < 1e-12);
	EXPECT_TRUE(std::abs(rcp_static_cast<const ComplexDouble>(r2)->i.imag() + 0.0791855203619909) < 1e-12);
}

TEST(symengineArit, Pow)
{
	...
	r1 = pow(div(i4, i6), i2);
	EXPECT_TRUE(eq(*r1, *div(integer(4), integer(9))));
	...
	r2 = pow(r1, i3);
	EXPECT_TRUE(eq(*r2, *integer(81)));

	r1 = real_double(0.1);
	r2 = Rational::from_mpq(mpq_class(1, 2));
	r2 = pow(r1, r2);
	EXPECT_TRUE(is_a<RealDouble>(*r2));
	EXPECT_TRUE(std::abs(rcp_static_cast<const RealDouble>(r2)->i - 0.316227766016) < 1e-12);
	r2 = pow(pow(r2, integer(3)), real_double(0.2));
	EXPECT_TRUE(std::abs(rcp_static_cast<const RealDouble>(r2)->i - 0.501187233627) < 1e-12);

	r1 = real_double(-0.01);
	r2 = pow(r1, Rational::from_mpq(mpq_class(1, 2)));
	r2 = pow(integer(2), r2);
	EXPECT_TRUE(is_a<ComplexDouble>(*r2));
	EXPECT_TRUE(std::abs(rcp_static_cast<const ComplexDouble>(r2)->i.real() - 0.997598696589298) < 1e-12);
	EXPECT_TRUE(std::abs(rcp_static_cast<const ComplexDouble>(r2)->i.imag() - 0.069259227279362) < 1e-12);
...
/*
TEST(symengineArit, Multinomial)
{
	map_vec_int r;
...
TEST(symengineArit, Expand2)
{
	...
	EXPECT_TRUE(eq(*r2, *add(add(mul(w, x), mul(w, y)), mul(w, z))));
	EXPECT_TRUE(neq(*r2, *add(add(mul(w, x), mul(w, w)), mul(w, z))));

	r1 = mul(add(x, y), add(z, w)); // (x+y)*(z+w)

	r2 = expand(r1);

	EXPECT_TRUE(eq(*r2, *add(add(add(mul(x,z), mul(y, z)), mul(x, w)), mul(y, w))));
	EXPECT_TRUE(neq(*r2, *add(add(add(mul(y,z), mul(y,z)), mul(x, w)), mul(y, w))));

	r1 = pow(add(x, y), im1);       // 1/(x+y)

	r2 = expand(r1);

	EXPECT_TRUE(eq(*r2, *r1));

	r1 = pow(add(x, y), im2);       // 1/(x+y)^2

	r2 = expand(r1);

	EXPECT_TRUE(eq(*r2, *pow(add(add(pow(x, i2), mul(mul(i2, x), y)), pow(y, i2)), im1)));
	EXPECT_TRUE(neq(*r2, *r1));
	...
	EXPECT_TRUE(eq(*r2, *add(div(integer(54), i5), mul(integer(14), pow(i5, div(im1, i2))))));
	...
	r1 = add(r1, x);
	r2 = expand(pow(r1, i2));
	EXPECT_TRUE(is_a<Add>(*r2));
	auto it = rcp_static_cast<const Add>(r2)->dict_.find(x);
	EXPECT_TRUE(it != rcp_static_cast<const Add>(r2)->dict_.end());
	EXPECT_TRUE(is_a<RealDouble>(*it->second));
	EXPECT_TRUE(std::abs(rcp_static_cast<const RealDouble>(it->second)->i - 0.4) < 1e-12);
