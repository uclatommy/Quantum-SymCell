...
TEST(symenginePrinting, test_printing)
{
    ...
    EXPECT_TRUE(r->__str__() == "(3/49)*196**(1/2)");

    r = mul(integer(12), pow(integer(196), div(integer(1), integer(2))));
    EXPECT_TRUE(r->__str__() == "12*196**(1/2)");

    r = mul(integer(23), mul(pow(integer(5), div(integer(1), integer(2))),
        pow(integer(7), div(integer(1), integer(2)))));
    EXPECT_TRUE(r->__str__() == "23*5**(1/2)*7**(1/2)");

    r = mul(integer(2), pow(RCP<const Basic>(symbol("x")), integer(2)));
    EXPECT_TRUE(r->__str__() == "2*x**2");
    ...
    EXPECT_TRUE(r->__str__() == "-33");

    r1 = mul(x, i);
    r2 = mul(r1, y);
    EXPECT_TRUE(r1->__str__() == "-x");
    EXPECT_TRUE(r1->__str__() != "-1x");
    EXPECT_TRUE(r2->__str__() == "-x*y");
    EXPECT_TRUE(r2->__str__() != "-1x*y");

    r = mul(integer(-1), pow(integer(196), div(integer(1), integer(2))));
    EXPECT_TRUE(r->__str__() == "-196**(1/2)");
    r = pow(integer(-6), div(integer(1), integer(2)));
    EXPECT_TRUE(r->__str__() == "(-6)**(1/2)");
    ...
    r1 = mul(c1, x);
    r2 = mul(c2, x);
    EXPECT_TRUE(c1->__str__() == "1/2 + 5/7*I");
    EXPECT_TRUE(c2->__str__() == "1/2 - 5/7*I");
    EXPECT_TRUE(r1->__str__() == "(1/2 + 5/7*I)*x");
    EXPECT_TRUE(r2->__str__() == "(1/2 - 5/7*I)*x");
    r1 = pow(RCP<const Basic>(x), c1);
    r2 = pow(RCP<const Basic>(x), c2);
    EXPECT_TRUE(r1->__str__() == "x**(1/2 + 5/7*I)");
    EXPECT_TRUE(r2->__str__() == "x**(1/2 - 5/7*I)");
    ...
    r1 = mul(c1, x);
    r2 = mul(c2, x);
    EXPECT_TRUE(c1->__str__() == "5/7*I");
    EXPECT_TRUE(c2->__str__() == "-5/7*I");
    EXPECT_TRUE(r1->__str__() == "5/7*I*x");
    EXPECT_TRUE(r2->__str__() == "-5/7*I*x");
    r1 = pow(RCP<const Basic>(x), c1);
    r2 = pow(RCP<const Basic>(x), c2);
    EXPECT_TRUE(r1->__str__() == "x**(5/7*I)");
    EXPECT_TRUE(r2->__str__() == "x**(-5/7*I)");

    c1 = Complex::from_two_nums(*rn2, *rn1);
    c2 = Complex::from_two_nums(*rn3, *rn1);
    r1 = mul(c1, x);
    r2 = mul(c2, x);
    EXPECT_TRUE(c1->__str__() == "5/7");
    EXPECT_TRUE(c2->__str__() == "-5/7");
    EXPECT_TRUE(r1->__str__() == "(5/7)*x");
    EXPECT_TRUE(r2->__str__() == "(-5/7)*x");
    r1 = pow(RCP<const Basic>(x), c1);
    r2 = pow(RCP<const Basic>(x), c2);
    EXPECT_TRUE(r1->__str__() == "x**(5/7)");
    EXPECT_TRUE(r2->__str__() == "x**(-5/7)");
    ...
    r1 = mul(integer(2), x);
    EXPECT_TRUE(r1->__str__() == "2*x" );

    r1 = mul(mul(integer(2), pow(symbol("x"), div(integer(2), integer(3)))), y);
    EXPECT_TRUE(r1->__str__() == "2*x**(2/3)*y" );

    r1 = mul(x, y);
    EXPECT_TRUE(r1->__str__() == "x*y" );

    r = div(x, add(x, y));
    r1 = div(x, pow(add(x, y), div(integer(2), integer(3))));
    r2 = div(x, pow(add(x, y), div(integer(-2), integer(3))));
    EXPECT_TRUE(r->__str__() == "x/(x + y)");
    EXPECT_TRUE(r1->__str__() == "x/(x + y)**(2/3)");
    EXPECT_TRUE(r2->__str__() == "x*(x + y)**(2/3)");

    r = div(integer(1), mul(x, add(x, y)));
    r1 = div(mul(y, integer(-1)), mul(x, add(x, y)));
    r2 = mul(pow(RCP<const Basic>(y), x), pow(RCP<const Basic>(x), y));
    EXPECT_TRUE(r->__str__() == "1/(x*(x + y))");
    EXPECT_TRUE(r1->__str__() == "-y/(x*(x + y))");
    EXPECT_TRUE(r2->__str__() == "x**y*y**x");

    r = pow(y, pow(RCP<const Basic>(x), integer(2)));
    r1 = pow(integer(3), mul(integer(2), x));
    r2 = pow(integer(3), mul(integer(-1), x));
    EXPECT_TRUE(r->__str__() == "y**(x**2)");
    EXPECT_TRUE(r1->__str__() == "3**(2*x)" );
    EXPECT_TRUE(r2->__str__() == "3**(-x)" );

    r1 = pow(mul(integer(2), x), y);
    r2 = pow(mul(x, y), z);
    EXPECT_TRUE(r1->__str__() == "(2*x)**y");
    EXPECT_TRUE(r2->__str__() == "(x*y)**z");

    r1 = pow(pow(RCP<const Basic>(integer(2)), x), y);
    r2 = pow(pow(RCP<const Basic>(x), y), z);
    EXPECT_TRUE(r1->__str__() == "(2**x)**y");
    EXPECT_TRUE(r2->__str__() == "(x**y)**z");

    r = pow(RCP<const Basic>(I), x);
    r1 = sub(sub(integer(2), x), y);
    EXPECT_TRUE(r->__str__() == "I**x");
    EXPECT_TRUE(r1->__str__() == "2 - x - y");
    ...
    EXPECT_TRUE(r->__str__() == "Derivative(f(x), x)");
    EXPECT_TRUE(r1->__str__() == "Derivative(f(x), x)");
    EXPECT_TRUE(r2->__str__() == "Derivative(g(x), x)");

    r1 = f->diff(x)->diff(x);
    EXPECT_TRUE(r1->__str__() == "Derivative(f(x), x, x)");

    f = function_symbol("f", {x, y});
    r = f->diff(x)->diff(y);
    r1 = Derivative::create(f, {x, y});
    r2 = Derivative::create(f, {y, x});
    EXPECT_TRUE(r->__str__() == "Derivative(f(x, y), x, y)");
    EXPECT_TRUE(r1->__str__() == "Derivative(f(x, y), x, y)");
    EXPECT_TRUE(r2->__str__() == "Derivative(f(x, y), y, x)");

    r1 = Subs::create(Derivative::create(function_symbol("f", {y, x}), {x}), {{x, add(x, y)}});
    EXPECT_TRUE(r1->__str__() == "Subs(Derivative(f(y, x), x), (x), (x + y))");
}

TEST(symenginePrinting, test_matrix)
{
    DenseMatrix A = DenseMatrix(2, 2, {integer(1), integer(0), integer(0),
        integer(1)});
    EXPECT_TRUE(A.__str__() == "[1, 0]\n[0, 1]\n");
}

TEST(symenginePrinting, test_univariate_polynomial)
{
    RCP<const Basic> p;
    RCP<const Symbol> x = symbol("x");

    p = univariate_polynomial(x, 0, {{0, 0}});
    EXPECT_TRUE(p->__str__() == "0");
    //std::cout<<p->__str__()<<std::endl;
    p = univariate_polynomial(x, 0, {{0, 1}});
    EXPECT_TRUE(p->__str__() == "1");
    //std::cout<<p->__str__()<<std::endl;
    p = univariate_polynomial(x, 1, {{1, 1}});
    EXPECT_TRUE(p->__str__() == "x");
    //std::cout<<p->__str__()<<std::endl;
    p = univariate_polynomial(x, 1, {{0, 1}, {1, 2}});
    EXPECT_TRUE(p->__str__() == "2*x + 1");
    //std::cout<<p->__str__()<<std::endl;
    p = univariate_polynomial(x, 1, {{0, -1}, {1, 2}});
    EXPECT_TRUE(p->__str__() == "2*x - 1");
    //std::cout<<p->__str__()<<std::endl;
    ...
    EXPECT_TRUE(p->__str__() == "x - 1");
    //std::cout<<p->__str__()<<std::endl;
    p = univariate_polynomial(x, 2, {{0, 1}, {1, 1}, {2, 1}});
    EXPECT_TRUE(p->__str__() == "x**2 + x + 1");
    //std::cout<<p->__str__()<<std::endl;
    p = univariate_polynomial(x, 2, {{0, 1}, {1, -1}, {2, 1}});
    EXPECT_TRUE(p->__str__() == "x**2 - x + 1");
    //std::cout<<p->__str__()<<std::endl;
...
TEST(symenginePrinting, test_floats)
{
    RCP<const Basic> p;;
    ...
    p = pow(p, x);
    EXPECT_TRUE(p->__str__() == "11111.11**x");
    ...
    p = mul(p, x);
    EXPECT_TRUE(p->__str__() == "(0.1 + 0.2*I)*x");

    p = real_double(123);
    p = sub(p, x);
    EXPECT_TRUE(p->__str__() == "123.0 - x");

    p = complex_double(std::complex<double>(1, 2));
    p = add(p, x);
    EXPECT_TRUE(p->__str__() == "1.0 + 2.0*I + x");
    ...
    p = add(p, x);
    EXPECT_TRUE(p->__str__() == "123.000000000000000000000 + x");
#ifdef HAVE_SYMENGINE_MPC
    SymEngine::mpc_class m2(75);
    mpc_set_si_si(m2.get_mpc_t(), -10, 10, MPC_RNDNN);
    p = SymEngine::complex_mpc(m2);
    p = div(p, x);
    EXPECT_TRUE(p->__str__() == "(-10.0000000000000000000000 + 10.0000000000000000000000*I)/x");
#endif
#endif
}
